...
for name, url in zip(path_random, path_results):
    ...
    for dataset in os.listdir(url):
        metrics[name][dataset] = pd.read_csv(url / Path(dataset) / "pij" / Path("pij_true" + ext_type), index_col=0)
        metrics[name][dataset] = (
            metrics[name][dataset].mean(axis=0).sort_values(ascending=False).to_frame(name="average_response")
        )
