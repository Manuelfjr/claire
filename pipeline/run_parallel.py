...
def execute_code(i_random):
    path_result = Path(config.dir_result)
    which_k_random = "n_random_model: [ {} ]".format(i_random + 1)
    print()
    print(title_part_n1 + which_k_random + title_part_n3)
    if number_random_models != 1:
        path_result = path_result / Path(f"random_n{i_random+1}")
    ...
    for i in tqdm(config.file_names):
        ...
        pij = claire.generate_pij_matrix(data_results, i_random + 1, n_clusters)
...
# Número de processos a serem executados em paralelo
num_processes = 4

# Lista para armazenar os processos
processes = []

# Loop for paralelizado
for k_random in range(number_random_models):
    # Cria um novo processo
    process = multiprocessing.Process(target=execute_code, args=(k_random,))
