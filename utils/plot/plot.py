...
class GeneratePlots:
    ...
    def scatterplot(self, nrows=1, ncols=1, figsize=(18, 18), plot_parameters=None):
        _fig = {}
        for which_param in plot_parameters:
            ...
            # axes.set_title(name)
            axes.set_xlabel("$pca_{(1)}$")
            axes.set_ylabel("$pca_{(2)}$")
    ...
    def scatterplot_diff_disc_unique(self, nrows=5, ncols=2, figsize=(18, 14), plot_parameters=None):
        fig, axes = plt.subplots(nrows, ncols, figsize=figsize)
        if isinstance(axes, matplotlib.axes._axes.Axes):
            axes = np.array([[axes]])
        elif axes.shape == (2,):
        ...
        for idxx1, _name in enumerate(self.data.keys()):
            for idxx2, args in enumerate(plot_parameters[_name]):
                ...
                axes[ax[1], ax[0]].set_title(f"{_name}", fontsize=20)
                axes[ax[1], ax[0]].set_ylabel(f"{name}", fontsize=20)
